...
class gender_rec_Brain(sb.Brain):
    ...
    def evaluate(
        ...
        import pickle
        with open("id_order.pkl", "wb") as f:
            pickle.dump(ids, f)
...
if __name__ == "__main__":
    ...
    recorder = torchfunc.hooks.recorders.ForwardPre(reduction=lambda x, y: torch.cat((x, y), 0))
    # Record inputs going into Linear(100, 50) and Linear(50, 10)
    recorder.children(gender_rec_brain.modules.classifier.out)
    ...
    possibilities = recorder.data[0].tolist()
    with open("possibilities.pkl", "wb") as f:
        pickle.dump(possibilities, f)
